...
def save_video(
    ...
    Uses a temporary file to avoid issues with network-mounted filesystems
    (e.g., OSS, S3 fuse) that don't support random write operations needed
    by video encoding.
    ...
    # Write to temp file first to avoid issues with network-mounted filesystems
    # (OSS/S3 fuse mounts don't support random writes needed for video encoding)
    with tempfile.NamedTemporaryFile(suffix=".mp4", delete=False) as tmp_file:
        tmp_path = tmp_file.name

    try:
        with av.open(tmp_path, mode="w") as container:
            # Setup video stream
