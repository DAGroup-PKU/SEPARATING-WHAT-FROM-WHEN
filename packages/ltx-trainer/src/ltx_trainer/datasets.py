...
class PrecomputedDataset(Dataset):
    def __init__(self, data_root: str, data_sources: dict[str, str] | list[str] | None = None) -> None:
        ...
        self.num_frames_list: list[int] = self._load_frame_counts()
    ...
    def _discover_samples(self) -> dict[str, list[Path]]:
        ...
        # Stable order across ranks/machines so index i always maps to the same clip and
        # num_frames_list[i] stays aligned with BucketBatchSampler buckets.
    ...
    def _load_frame_counts(self) -> list[int]:
        """Load per-sample latent frame counts from frame_counts.json (written by process_videos.py).
        Falls back to 0 for missing entries (sampler will treat them as unknown length)."""
        import json as _json
        latent_dir = self.source_paths.get("latents", next(iter(self.source_paths.values())))
        index_path = latent_dir / "frame_counts.json"
        if index_path.exists():
            index = _json.loads(index_path.read_text())
        else:
            logger.warning(
                f"frame_counts.json not found in {latent_dir}. "
                "BucketBatchSampler will fall back to random sampling. "
                "Re-run process_dataset.py to generate it."
            )
            index = {}
        first_key = next(iter(self.sample_files.keys()))
        return [index.get(Path(p).stem, 0) for p in self.sample_files[first_key]]
