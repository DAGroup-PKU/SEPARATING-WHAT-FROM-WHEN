...
class LossOutput:
    """Per-element loss + components for logging.

    ``total`` is the tensor backward() runs on (video + audio when audio is on).
    ``video`` / ``audio`` are the same per-element tensors before being summed,
    exposed so the trainer can log them separately. ``audio`` is ``None`` when
    the strategy is video-only or audio is disabled.
    """

    total: Tensor  # [B,] used for backward
    video: Tensor  # [B,] always present
    audio: Tensor | None  # [B,] None when audio disabled
...
class TrainingStrategy(ABC):
    ...
    def compute_loss(
        ...
        Returns:
            ``LossOutput`` with ``total`` ([B,]) used for backward and per-sigma
            bucket tracking, plus ``video`` / ``audio`` components for logging.
        """
