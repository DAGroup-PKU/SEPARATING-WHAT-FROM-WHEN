...
class TextToVideoConfig(TrainingStrategyConfigBase):
    ...
    audio_loss_weight: float = Field(
        default=1.0,
        description="Weight applied to audio MSE before summing into the total loss "
        "(total = video_loss + audio_loss_weight * audio_loss). Default 1.0 keeps "
        "the historical behavior. Set <1.0 when audio_loss is several times larger "
        "than video_loss and you want to rebalance the gradient contribution.",
        ge=0.0,
    )
...
class TextToVideoStrategy(TrainingStrategy):
    ...
    def prepare_training_inputs(
        ...
        context_positions = conditions.get("context_positions", None)
    ...
    def compute_loss(
        ...
        """Compute masked MSE loss for video and optionally audio. Returns per-element [B,] components."""
        ...
        if not self.config.with_audio or audio_pred is None or inputs.audio_targets is None:
            return LossOutput(total=video_loss, video=video_loss, audio=None)

        # Audio loss: per-element mean over (seq, channels), [B,]
        audio_loss = (audio_pred - inputs.audio_targets).pow(2).mean(dim=[-2, -1])

        # Combined loss [B,]. audio_loss_weight=1.0 (default) keeps historical
        # 1:1 sum; lower it (e.g. 0.5) when audio loss is consistently larger
        # than video and you want to rebalance the backward signal.
        total = video_loss + self.config.audio_loss_weight * audio_loss
        return LossOutput(total=total, video=video_loss, audio=audio_loss)
