...
from ltx_core.model.transformer.time_range_annotator import (
    annotate_and_pack_for_rote_v2,
    strip_time_ranges,
    make_no_rope_positions,
)
...
from ltx_trainer.bucketed_sampler import BucketBatchSampler, unwrap_bucket_batch_sampler
...
# Disable progress bars if not main process (use global RANK so only one process across all nodes is "main")
...
class TrainingStepOutput:
    ...
    loss: Tensor  # [B,] per-element loss (unreduced) — drives backward()
    sigma: Tensor  # [B,] sampled sigma, detached from computational graph
    video_loss: Tensor  # [B,] per-element video loss component (always present)
    audio_loss: Tensor | None  # [B,] per-element audio loss; None when audio disabled
...
class LtxvTrainer:
    ...
    def train(  # noqa: PLR0912, PLR0915
        ...
        # Per-epoch loss tracking for TensorBoard. Step-level loss is logged
        # every optimization step; epoch-level loss is logged on the boundary
        # detected via StopIteration (and once at the end for the final partial
        # epoch). Tracked on the local main process only — matches step_loss.
        epoch_loss_sum = 0.0
        epoch_loss_count = 0
        current_epoch = 0
        ...
        with progress:
            ...
            for step in range(remaining_steps * cfg.optimization.gradient_accumulation_steps):
                ...
                try:
                    ...
                    # Epoch boundary: log per-epoch averaged loss before advancing.
                    if (
                        self._accelerator.is_local_main_process
                    ...
                    # Advance inner BucketBatchSampler epoch (BatchSamplerShard has no set_epoch).
                    bucket_sampler = unwrap_bucket_batch_sampler(self._dataloader)
                    if bucket_sampler is not None:
                        bucket_sampler.set_epoch(bucket_sampler.epoch + 1)
                    else:
                        raw_sampler = getattr(self._dataloader, "batch_sampler", None) or getattr(
                            self._dataloader, "sampler", None
                        )
                        if hasattr(raw_sampler, "set_epoch"):
                            raw_sampler.set_epoch(getattr(raw_sampler, "epoch", 0) + 1)
                    data_iter = iter(self._dataloader)
                    batch = next(data_iter)
                ...
                with self._accelerator.accumulate(self._transformer):
                    ...
                    if self._accelerator.sync_gradients and cfg.optimization.max_grad_norm > 0:
                        # clip_grad_norm_ 返回 clip 前的 total L2 norm,记录到 metrics 里。
                    ...
                    group_lrs = [g["lr"] for g in self._optimizer.param_groups]
                    ...
                    step_video_loss = output.video_loss.detach().mean().item()
                    step_audio_loss = (
                        output.audio_loss.detach().mean().item() if output.audio_loss is not None else None
                    )
                    ...
                    # Log metrics to W&B and TensorBoard (only on main process and optimization steps)
                    if IS_MAIN_PROCESS and is_optimization_step:
                        ...
                        metrics = {
                            ...
                            "train/video_loss": step_video_loss,
                            "train/learning_rate": current_lr,
                            "train/step_time": step_time,
                            "train/global_step": self._global_step,
                        }
                        if step_audio_loss is not None:
                            metrics["train/audio_loss"] = step_audio_loss
                        if grad_norm is not None:
                            # 记 clip 前的 total L2 norm (across all trainable params)。
                            # 观察 grad_norm 长期是否稳定 / 是否被频繁 clip / 训练是否
                            # 发散 (grad_norm 暴涨) 都靠这个。
                            metrics["train/grad_norm"] = grad_norm.item() if hasattr(grad_norm, "item") else float(grad_norm)
                        for i, lr_i in enumerate(group_lrs):
                            metrics[f"train/learning_rate_g{i}"] = lr_i
                        metrics.update(self._sigma_tracker.get_metrics())
                        self._log_metrics(metrics)

                    # Log to TensorBoard (use accelerator.is_local_main_process for reliable detection)
                    if self._accelerator.is_local_main_process and is_optimization_step:
                        if self._tb_writer is not None:
                            self._tb_writer.add_scalar("train/loss", step_loss, self._global_step)
                            self._tb_writer.add_scalar("train/video_loss", step_video_loss, self._global_step)
                            if step_audio_loss is not None:
                                self._tb_writer.add_scalar("train/audio_loss", step_audio_loss, self._global_step)
                            self._tb_writer.add_scalar("train/learning_rate", current_lr, self._global_step)
                            for i, lr_i in enumerate(group_lrs):
                                self._tb_writer.add_scalar(
                                    f"train/learning_rate_g{i}", lr_i, self._global_step
                                )
                            self._tb_writer.add_scalar("train/step_time", step_time, self._global_step)
                            if grad_norm is not None:
                                gn_val = grad_norm.item() if hasattr(grad_norm, "item") else float(grad_norm)
                                self._tb_writer.add_scalar("train/grad_norm", gn_val, self._global_step)

                    # Accumulate for per-epoch loss (every fwd/bwd, including
                    # accumulation sub-steps — denominator stays consistent).
        ...
        # Flush the final partial epoch's averaged loss (training may stop
        # mid-epoch when total step budget is reached before StopIteration).
        if (
            self._accelerator.is_local_main_process
            and self._tb_writer is not None
            and epoch_loss_count > 0
        ...
        # Close TensorBoard writer (use accelerator check, not IS_MAIN_PROCESS)
        if self._accelerator.is_local_main_process and self._tb_writer is not None:
    ...
    def _training_step(self, batch: dict[str, dict[str, Tensor]]) -> TrainingStepOutput:
        ...
        loss_out = self._training_strategy.compute_loss(video_pred, audio_pred, model_inputs)
        sigma = model_inputs.video.sigma.detach() if model_inputs.video.enabled else model_inputs.audio.sigma.detach()

        return TrainingStepOutput(
            loss=loss_out.total,
            sigma=sigma,
            video_loss=loss_out.video,
            audio_loss=loss_out.audio,
        )
    ...
    def _load_text_encoder_and_cache_embeddings(self) -> list[CachedPromptEmbeddings] | None:
        ...
        # Embeddings processor (connectors) is required for every training step.
        # Gemma itself is only needed to encode validation prompts.
        ...
        if self._config.validation.prompts:
            ...
            with torch.inference_mode():
                for prompt in self._config.validation.prompts:
                    # Strip "time_range": [...] before tokenization (matches training-side
                    # process_captions.py and inference-side validation_sampler.py).
                    # Time info still reaches the model via RoTE from timing_map.
                    stripped_prompt, timing_map = strip_time_ranges(prompt)

                    pos_hs, pos_mask = text_encoder.encode(stripped_prompt)
                    pos_out = self._embeddings_processor.process_hidden_states(pos_hs, pos_mask)

                    stripped_neg, _ = strip_time_ranges(self._config.validation.negative_prompt)
                    neg_hs, neg_mask = text_encoder.encode(stripped_neg)
                    neg_out = self._embeddings_processor.process_hidden_states(neg_hs, neg_mask)

                    # Use the pre-connector mask length as seq_len: annotate_and_pack_for_rote_v2
                    # internally tokenizes with this length to get the same attention mask the
                    # connector saw, then mirrors the connector's left-pack permutation.
                    seq_len = pos_mask.shape[1]
                    ctx_pos = annotate_and_pack_for_rote_v2(
                        stripped_prompt, text_encoder.tokenizer, seq_len, timing_map
                    ).unsqueeze(0)
                    # Negative prompt is a generic quality string ("worst quality, ...") with no
                    # event/shot timeline — every K token bypasses RoTE.
                    ctx_pos_neg = make_no_rope_positions(seq_len)
                    ...
                    cached_embeddings.append(
                        CachedPromptEmbeddings(
                            ...
                            context_positions=ctx_pos.cpu(),
                            context_positions_negative=ctx_pos_neg.cpu(),
                        )
                    )
            ...
            logger.info("No validation prompts configured; skipping Gemma load.")

        # Training uses precomputed caption features + connectors only.
    ...
    def _load_models(self) -> None:
        ...
        # Apply yaml-level RoTE overrides (gamma/alpha) onto the transformer's
        # args preprocessors, which is where rope.precompute_rote_freqs_cis reads
        # them from at every forward pass. The configurator only sees checkpoint
        # config; the trainer config supersedes that here.
        self._apply_temporal_overrides()
    ...
    def _apply_temporal_overrides(self) -> None:
        """Push yaml-level TCR / temporal-operator settings onto the preprocessors.

        ``LTXModelConfigurator`` only sees the checkpoint's transformer config,
        so trainer yaml values have to be threaded in post-load. Both
        AudioVideo (MultiModalTransformerArgsPreprocessor) and single-modality
        (TransformerArgsPreprocessor) layouts are handled.
        """
        gamma = self._config.model.rote_gamma
        alpha = self._config.model.rote_alpha
        temporal_mode = self._config.model.temporal_mode
        tcr_beta = self._config.model.tcr_beta
        tcr_alpha = self._config.model.tcr_alpha
        if (
            gamma is None
            and alpha is None
            and temporal_mode is None
            and tcr_beta is None
            and tcr_alpha is None
        ):
            return

        targets = []
        for name in ("video_args_preprocessor", "audio_args_preprocessor"):
            preproc = getattr(self._transformer, name, None)
            if preproc is None:
                continue
            target = getattr(preproc, "simple_preprocessor", preproc)
            targets.append((name, target))

        for name, target in targets:
            if gamma is not None:
                target.rote_gamma = gamma
                self._transformer.rote_gamma = gamma
            if alpha is not None:
                target.rote_alpha = alpha
                self._transformer.rote_alpha = alpha
            if temporal_mode is not None:
                target.temporal_mode = temporal_mode
                self._transformer.temporal_mode = temporal_mode
            if tcr_beta is not None:
                target.tcr_beta = tcr_beta
                self._transformer.tcr_beta = tcr_beta
            if tcr_alpha is not None:
                target.tcr_alpha = tcr_alpha
                self._transformer.tcr_alpha = tcr_alpha
            logger.info(
                f"Temporal override applied to {name}: "
                f"mode={target.temporal_mode}, "
                f"tcr_beta={getattr(target, 'tcr_beta', 'N/A')}, "
                f"tcr_alpha={getattr(target, 'tcr_alpha', 'N/A')}"
            )
    ...
    def _init_dataloader(self) -> None:
        ...
        # Bucketed batching: each batch (and each DDP step across GPUs) uses the same
        # latent frame count so all ranks process clips of the same length.
        num_frames_list = getattr(self._dataset, "num_frames_list", None)
        if num_frames_list and any(n > 0 for n in num_frames_list):
            batch_sampler = BucketBatchSampler(
                num_frames_list=num_frames_list,
                batch_size=batch_size,
                seed=self._config.seed if hasattr(self._config, "seed") else 42,
                drop_last=True,
            )
            logger.info(
                "Using BucketBatchSampler: same temporal tier per batch and per DDP step."
            )
            ...
            logger.warning(
                "frame_counts.json not found – falling back to random shuffle. "
                "Re-run process_dataset.py to enable bucketed sampling."
            )
            dataloader = DataLoader(
                self._dataset,
                batch_size=self._config.optimization.batch_size,
    ...
    def _init_optimizer(self) -> None:
        ...
        # Per-pattern lr groups (e.g. smaller lr for audio_attn / audio_ff).
        # Patterns are regex; matched via re.search on the full parameter name.
        # First match wins; unmatched params fall back to the main lr group.
        if opt_cfg.learning_rate_groups:
            param_groups = [{"params": [], "lr": lr, "names": []}]
            pattern_to_idx = {}
            for i, (pattern, group_lr) in enumerate(opt_cfg.learning_rate_groups.items()):
                param_groups.append({"params": [], "lr": group_lr, "names": []})
                pattern_to_idx[pattern] = i + 1

            for name, param in self._transformer.named_parameters():
                if not param.requires_grad:
                    continue
                assigned = False
                for pattern, idx in pattern_to_idx.items():
                    if re.search(pattern, name):
                        param_groups[idx]["params"].append(param)
                        param_groups[idx]["names"].append(name)
                        assigned = True
                        break
                if not assigned:
    ...
    def _create_scheduler(self, optimizer: torch.optim.Optimizer) -> LRScheduler | None:
        ...
        Supports an optional ``warmup_steps`` in ``scheduler_params``. When > 0,
        a linear warmup (lr*0.01 → lr) is prepended via SequentialLR. The main
        scheduler's total_iters is shortened by ``warmup_steps`` so the combined
        schedule still finishes at ``steps``.
        """
        ...
        main_iters = max(steps - warmup_steps, 1)
        ...
        if scheduler_type == "linear":
            ...
            main_scheduler = CosineAnnealingWarmRestarts(
                optimizer,
                T_0=params.pop("T_0", main_iters // 4),
            ...
            main_scheduler = StepLR(
                optimizer,
                step_size=params.pop("step_size", main_iters // 2),
        ...
        if warmup_steps > 0 and main_scheduler is not None:
            ...
            logger.info(
                f"LR scheduler: {warmup_steps}-step linear warmup → {scheduler_type} "
                f"({main_iters} steps); total {steps} steps."
            )
            return scheduler
    ...
    def _init_tensorboard(self) -> None:
        """Initialize TensorBoard writer (only on local main process)."""
        if not self._accelerator.is_local_main_process:
            return

        tb_log_dir = str(Path(self._config.output_dir) / "tensorboard")
