...
class ModelConfig(ConfigBaseModel):
    ...
    rote_gamma: float | None = Field(
        default=None,
        description="Override RoTE time-scaling factor (text cross-attention). "
        "Phases are computed as q_time·γ·freqs (Q) and c·γ·freqs / r·γ·freqs (K). "
        "TIE paper default is 4.0. For ≈8s raw-second clips, 1.0 is gentler "
        "(≈1.27 cycles vs 5.09 at γ=4) and easier on a pretrained cross-attn "
        "that has no prior RoPE exposure.",
    )

    rote_alpha: float | None = Field(
        default=None,
        description="Override RoTE sinc bandwidth multiplier (text cross-attention). "
        "Default 1.0 follows TIE; lower values widen the K interval's effective "
        "temporal support (slower sinc decay), higher values sharpen it.",
    )

    temporal_mode: str | None = Field(
        default="tcr",
        description="Temporal operator for text cross-attention. "
        "'tcr' (paper default): Temporal Context Routing, an interval-derived "
        "bias added to video--text and audio--text logits. "
        "'rotary': RoTE baseline (Q/K rotation, not used by the paper method). "
        "Legacy alias: 'additive' is accepted and mapped to 'tcr'.",
    )

    tcr_beta: float | None = Field(
        default=5.0,
        description="TCR scale β in B = -β α (t-c)^2 / (2 r^2). "
        "Paper default is 5.0 (endpoint retention −β/2 = −2.5).",
    )

    tcr_alpha: float | None = Field(
        default=1.0,
        description="Optional TCR width multiplier α. Paper default is 1.0.",
    )

    @model_validator(mode="before")
    @classmethod
    def remap_legacy_additive_keys(cls, data: object) -> object:
        """Accept the pre-paper names used in internal training yaml.

        rote_mode=additive → temporal_mode=tcr
        tab_scale → tcr_beta
        tab_alpha → tcr_alpha
        """
        if not isinstance(data, dict):
            return data
        out = dict(data)
        if "temporal_mode" not in out and "rote_mode" in out:
            out["temporal_mode"] = out.pop("rote_mode")
        else:
            out.pop("rote_mode", None)
        if "tcr_beta" not in out and "tab_scale" in out:
            out["tcr_beta"] = out.pop("tab_scale")
        else:
            out.pop("tab_scale", None)
        if "tcr_alpha" not in out and "tab_alpha" in out:
            out["tcr_alpha"] = out.pop("tab_alpha")
        else:
            out.pop("tab_alpha", None)
        return out

    @field_validator("temporal_mode")
    @classmethod
    def normalize_temporal_mode(cls, v: str | None) -> str | None:
        """Map the legacy training-code name 'additive' to the paper name TCR."""
        if v is None:
            return v
        key = str(v).strip().lower()
        aliases = {"additive": "tcr", "tab": "tcr", "tcr": "tcr", "rotary": "rotary"}
        if key not in aliases:
            raise ValueError(f"Unknown temporal_mode={v!r}. Expected 'tcr' or 'rotary'.")
        return aliases[key]

    @field_validator("model_path")
    @classmethod
    def validate_model_path(cls, v: str | Path) -> str | Path:
        """Reject URLs. Existence is checked at load time so the yaml can ship
        with placeholders and callers can pass --model-path themselves."""
        text = str(v)
...
class OptimizationConfig(ConfigBaseModel):
    ...
    learning_rate_groups: dict[str, float] | None = Field(
        default=None,
        description="Optional mapping of parameter name regex patterns to learning rates. "
        "Parameters whose name matches a pattern (re.search) use that group's lr; "
        "parameters that match none fall back to the main learning_rate. "
        "Iteration order of the dict matters — first match wins.",
    )
