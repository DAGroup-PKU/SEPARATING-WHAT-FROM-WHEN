...
class CachedPromptEmbeddings:
    ...
    context_positions: Tensor | None = None  # [1, seq_len, 2] for RoTE (positive)
    context_positions_negative: Tensor | None = None  # [1, seq_len, 2] for CFG negative pass
...
class ValidationSampler:
    ...
    def _generate_standard(self, config: GenerationConfig, device: torch.device) -> tuple[Tensor, Tensor | None]:
        ...
        # Get context_positions for RoTE (from cache or compute on-the-fly).
        # Returns (positive, negative): the negative pass uses all-sentinel so
        # the unconditional branch bypasses RoTE entirely.
        ctx_pos_positive, ctx_pos_negative = self._get_context_positions(config, v_ctx_pos.shape[1])
        ...
        video_state, audio_state = self._run_denoising(
            ...
            a_ctx_neg=a_ctx_neg,
            device=device,
            context_positions=ctx_pos_positive,
            context_positions_negative=ctx_pos_negative,
        )
    ...
    def _generate_with_reference(self, config: GenerationConfig, device: torch.device) -> tuple[Tensor, Tensor | None]:
        ...
        # Get context_positions for RoTE (positive, negative)
        ctx_pos_positive, ctx_pos_negative = self._get_context_positions(config, v_ctx_pos.shape[1])
    ...
    def _run_denoising(
        ...
        device: torch.device,
        context_positions: Tensor | None = None,
        context_positions_negative: Tensor | None = None,
        ...
        ctx_pos = context_positions.to(device) if context_positions is not None else None
        ctx_neg = (
            context_positions_negative.to(device) if context_positions_negative is not None else None
        )
        ...
        # Gemma must already be on CPU (see _encode_prompts); clear cache before 22B lands.
        ...
        with torch.autocast(device_type=str(device).split(":")[0], dtype=torch.bfloat16):
            for step_idx, sigma in enumerate(sigmas[:-1]):
                ...
                # Apply CFG if guidance_scale != 1.0. The negative branch must use its own
                # context_positions (all-sentinel for "worst quality, ..." style negatives) so
                # the unconditional pass bypasses RoTE — otherwise the positive prompt's event
                # timeline leaks into the unconditional branch and CFG delta becomes incoherent.
                if cfg_guider.enabled() and v_ctx_neg is not None:
                    video_neg = replace(video, context=v_ctx_neg, context_positions=ctx_neg)
                    audio_neg = (
                        replace(audio, context=a_ctx_neg, context_positions=ctx_neg)
    ...
    def _get_context_positions(
        self, config: GenerationConfig, seq_len: int
    ) -> tuple[Tensor | None, Tensor | None]:
        """Get per-token (t_s, t_e) positions for RoTE text cross-attention.

        Returns (positive, negative). The negative pass uses an all-sentinel
        tensor: the negative prompt ("worst quality, ...") has no event timeline,
        so every K token must bypass RoTE — otherwise the CFG delta mixes the
        positive prompt's time anchors into the unconditional branch.
        """
        cached = config.cached_embeddings
        if cached is not None and cached.context_positions is not None:
            ctx_pos = cached.context_positions
            ctx_neg = (
                cached.context_positions_negative
                if cached.context_positions_negative is not None
                else make_no_rope_positions(ctx_pos.shape[1])
            )
            return ctx_pos, ctx_neg

        # Compute on-the-fly from the prompt text — needs the text encoder's tokenizer.
        if self._text_encoder is None:
            raise ValueError(
                "text_encoder is required to compute context_positions on-the-fly; "
                "either provide cached_embeddings.context_positions or keep the text encoder loaded."
            )
        # Strip "time_range": [...] from the prompt before annotation — matches
        # the training-side preprocessing (process_captions.py) so that the K
        # token layout the model sees at inference is the same one it saw during
        # LoRA training.
        stripped_prompt, timing_map = strip_time_ranges(config.prompt)
        positions = annotate_and_pack_for_rote_v2(
            stripped_prompt, self._text_encoder.tokenizer, seq_len, timing_map
        )
        ctx_pos = positions.unsqueeze(0)  # (1, seq_len, 2)
        ctx_neg = make_no_rope_positions(seq_len)
        return ctx_pos, ctx_neg

    def _offload_for_text_encode(self) -> None:
        """Ensure 22B transformer / VAEs are off GPU before Gemma encode.

        On 80GB cards, LTX-2.3 (~50GB+) and Gemma-12B (~24GB) cannot coexist.
        Single-GPU runs rely on this temporal multiplexing; defend against any
        accidental early .to(cuda) (e.g. PEFT / shared site-packages).
        """
        for module in (
            getattr(self, "_transformer", None),
            getattr(self, "_vae_decoder", None),
            getattr(self, "_vae_encoder", None),
            getattr(self, "_audio_decoder", None),
            getattr(self, "_vocoder", None),
        ):
    ...
    def _encode_prompts(
        ...
        The positive prompt is stripped of ``"time_range": [...]`` fields before
        tokenization — these chars produce ~8 tokens per shot/event/subtitle but
        carry no signal the model can use (numeric reasoning is weak; time info
        flows through RoTE K-rotation instead). Negative prompts are plain text
        and pass through unchanged (``strip_time_ranges`` tolerates non-JSON).
        """
        # Peak-VRAM schedule: Gemma alone on GPU, then offload before transformer denoise.
        self._offload_for_text_encode()

        self._text_encoder.to(device)
        self._embeddings_processor.to(device)

        stripped_pos, _ = strip_time_ranges(config.prompt)
        pos_hs, pos_mask = self._text_encoder.encode(stripped_pos)
        ...
        if config.guidance_scale != 1.0:
            stripped_neg, _ = strip_time_ranges(config.negative_prompt)
            neg_hs, neg_mask = self._text_encoder.encode(stripped_neg)
        ...
        # Move Gemma (+ connector) back to CPU and free cache for the 22B transformer.
        self._text_encoder.to("cpu")
        self._embeddings_processor.to("cpu")
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
