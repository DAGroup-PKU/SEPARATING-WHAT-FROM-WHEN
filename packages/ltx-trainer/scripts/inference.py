...
def main() -> None:  # noqa: PLR0912, PLR0915
    ...
    # Temporal-operator overrides (must match the trainer yaml the LoRA was
    # trained with). Checkpoint config does not persist these, so the loaded
    # transformer otherwise falls back to LTXModelConfigurator defaults.
    parser.add_argument(
        "--temporal-mode",
        type=str,
        default=None,
        choices=["tcr", "rotary"],
        help="Temporal operator. 'tcr' is Temporal Context Routing (paper default). "
        "'rotary' is the RoTE baseline. Default: None → model default (tcr).",
    )
    parser.add_argument(
        "--tcr-beta",
        type=float,
        default=None,
        help="TCR scale β. Paper default: 5.0.",
    )
    parser.add_argument(
        "--rote-gamma",
        type=float,
        default=None,
        help="Override transformer.rote_gamma post-load (rotary baseline only).",
    )
    parser.add_argument(
        "--rote-alpha",
        type=float,
        default=None,
        help="Override transformer.rote_alpha post-load (rotary baseline only).",
    )
    ...
    if args.prompt and args.prompt_file:
        parser.error("Pass only one of --prompt or --prompt-file")
    ...
    # Load embeddings processor (needed for on-the-fly prompt encoding)
    ...
    # Apply temporal-operator overrides (must match the trainer yaml the LoRA
    # was trained with). Mirrors LtxvTrainer._apply_temporal_overrides.
    _need_temporal_override = any(
        v is not None
        for v in (
            args.rote_gamma,
            args.rote_alpha,
    ...
    if _need_temporal_override:
        for name in ("video_args_preprocessor", "audio_args_preprocessor"):
            preproc = getattr(transformer, name, None)
            if preproc is None:
                continue
            target = getattr(preproc, "simple_preprocessor", preproc)
            if args.rote_gamma is not None:
                target.rote_gamma = args.rote_gamma
                transformer.rote_gamma = args.rote_gamma
            if args.rote_alpha is not None:
                target.rote_alpha = args.rote_alpha
                transformer.rote_alpha = args.rote_alpha
            if args.temporal_mode is not None:
                target.temporal_mode = args.temporal_mode
                transformer.temporal_mode = args.temporal_mode
            if args.tcr_beta is not None:
                target.tcr_beta = args.tcr_beta
                transformer.tcr_beta = args.tcr_beta
            print(
                f"Temporal override on {name}: mode={target.temporal_mode}, "
    ...
    # Hard-pin everything on CPU before generate(). On 80GB H100, Gemma encode
    # and 22B denoise must be temporally multiplexed (see ValidationSampler).
    for _mod in (
        transformer,
        components.video_vae_decoder,
        components.video_vae_encoder,
        components.audio_vae_decoder,
        components.vocoder,
