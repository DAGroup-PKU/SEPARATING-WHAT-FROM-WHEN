        ...
        for batch in dataloader:
            ...
            with torch.inference_mode():
                ...
                for i in range(len(batch["prompt"])):
                    # Strip "time_range": [...] fields from the prompt before tokenization.
                    # Time info still reaches the model via RoTE (computed below from
                    # timing_map), while saving ~8 tokens per shot/event/subtitle that
                    # the LLM can't actually leverage (numeric reasoning is weak; the
                    # real signal is the K-side rotation, not the text "0.0, 2.0").
                    stripped_prompt, timing_map = strip_time_ranges(batch["prompt"][i])

                    hidden_states, prompt_attention_mask = text_encoder.encode(stripped_prompt, padding_side="left")
                    ...
                    seq_len = prompt_attention_mask.shape[1]
                    context_positions = annotate_and_pack_for_rote_v2(
                    ...
                    embedding_data = {
                        ...
                        "context_positions": context_positions.cpu().contiguous(),
                    }
