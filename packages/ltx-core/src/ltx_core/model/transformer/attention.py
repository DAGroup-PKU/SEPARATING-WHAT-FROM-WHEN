...
flash_attn_cute_func = None
try:
    from xformers.ops import memory_efficient_attention
except Exception:  # ImportError, or runtime triton/xformers version mismatch
    memory_efficient_attention = None
try:
    ...
    flash_attn_interface = None
try:
    # FlashAttention4 (cute) takes priority over FA3/XFormers when available
    from flash_attn.cute import flash_attn_func as _fa4_func
    flash_attn_cute_func = _fa4_func
except Exception:
    flash_attn_cute_func = None
...
class FlashAttention4(AttentionCallable):
    def __call__(
        ...
        if flash_attn_cute_func is None:
            raise RuntimeError("FlashAttention4 was selected but `flash_attn.cute` is not installed.")

        b, _, dim_head = q.shape
        dim_head //= heads

        if mask is not None:
            # FA4 does not support attention masks; fall back to PyTorch SDPA
            q2, k2, v2 = (t.view(b, -1, heads, dim_head).transpose(1, 2) for t in (q, k, v))
            if mask.ndim == 2:
                mask = mask.unsqueeze(0)
            if mask.ndim == 3:
                mask = mask.unsqueeze(1)
            out = torch.nn.functional.scaled_dot_product_attention(
                q2, k2, v2, attn_mask=mask, dropout_p=0.0, is_causal=False
            )
            return out.transpose(1, 2).reshape(b, -1, heads * dim_head)

        q, k, v = (t.view(b, -1, heads, dim_head) for t in (q, k, v))
        result = flash_attn_cute_func(q.to(v.dtype), k.to(v.dtype), v)
...
class AttentionFunction(Enum):
    ...
    def to_callable(self) -> AttentionCallable:
        ...
        if self is AttentionFunction.PYTORCH:
            ...
            # Default: FA4 (H100 CUTLASS) > FA3 > XFormers > PyTorch SDPA
            if flash_attn_cute_func is not None:
                return FlashAttention4()
