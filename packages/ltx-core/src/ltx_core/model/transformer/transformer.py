...
class BasicAVTransformerBlock(torch.nn.Module):
    ...
    def _apply_text_cross_attention(
        ...
        """Apply text cross-attention, with optional AdaLN and TCR temporal routing."""
        if temporal_bias is not None:
            if context_mask is not None:
                context_mask = context_mask + temporal_bias
            else:
                context_mask = temporal_bias
        ...
        return attn(rms_norm(x, eps=self.norm_eps), context=context, mask=context_mask, pe=pe, k_pe=k_pe)
    ...
    def forward(  # noqa: PLR0915
        ...
        if run_vx:
            ...
            vx = vx + self._apply_text_cross_attention(
                ...
                pe=video.text_cross_pe,
                k_pe=video.text_cross_k_pe,
                temporal_bias=video.text_cross_temporal_bias,
            )
        ...
        if run_ax:
            ...
            ax = ax + self._apply_text_cross_attention(
                ...
                audio.context_mask,
                cross_attention_adaln=self.cross_attention_adaln,
                pe=audio.text_cross_pe,
                k_pe=audio.text_cross_k_pe,
                temporal_bias=audio.text_cross_temporal_bias,
            )
...
def apply_cross_attention_adaln(
    ...
    return attn(attn_input, context=encoder_hidden_states, mask=context_mask, pe=pe, k_pe=k_pe) * q_gate
